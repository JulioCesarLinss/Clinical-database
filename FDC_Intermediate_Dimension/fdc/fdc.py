...
# Retorna o valor padrão caso o parâmetro não tenha sido informado
...
# Roda o UMAP convencional nos dados com a métrica escolhida e retorna embedding 2D
...
# Métrica de Canberra modificada para suportar valores negativos e ordinais
# Compilada com numba para melhor desempenho, pois é chamada milhares de vezes pelo UMAP
@jit(nopython=True)
...
# Aplica o UMAP em um grupo de features com a métrica e parâmetros configurados
class Clustering:
    def __init__(self, metric='euclidean', UMAP_neb=15, min_dist_UMAP=0.1, max_components=2):
        self.metric = metric
...
# Implementação do FDC: separa features por tipo, aplica UMAP com métrica adequada
# para cada grupo e concatena os embeddings parciais em uma representação unificada
class FDC:
    def __init__(self,
        ...
        # Cada tipo de feature usa sua própria métrica de distância
        self.clustering_cont = value(clustering_cont, Clustering('euclidean', 15, 0.1))
        self.clustering_ord = value(clustering_ord, Clustering(canberra_modified, 15, 0.1))
        self.clustering_nom = value(clustering_nom, Clustering('hamming', 15, 0.1, max_components=1))
    ...
    # Define quais colunas pertencem a cada tipo de feature
    ...
    # Gera os embeddings intermediário e 2D aplicando o pipeline do FDC
    def normalize(self, data, n_neighbors=15, min_dist=0.1,
                  cont_list=None, nom_list=None, ord_list=None,
                  with_2d_embedding=None, visual=None):
        ...
        # Para cada tipo de feature, aplica o UMAP com a métrica correspondente
        # e armazena o embedding parcial para concatenação posterior
        actions = [
        ...
        # Embedding intermediário: concatenação dos embeddings parciais de cada tipo
        result_concat = np.concatenate(concat_lists, axis=1)
        timing.step("concat")

        # Reduz o embedding intermediário para 2D para visualização
